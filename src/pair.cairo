...
mod StarkFinancePair {
    ...
    use starkfinance::interfaces::erc20::{ERC20ABIDispatcherTrait, ERC20ABIDispatcher};

    use starknet::syscalls::deploy_syscall;
    use starkfinance::utils::selectors;
    use starkfinance::utils::call_fallback::{
        UnwrapAndCast, call_contract_with_selector_fallback
    };
    use starkfinance::interfaces::factory::{
        IStarkFinanceFactoryABIDispatcher, IStarkFinanceFactoryABIDispatcherTrait
    };
    use starkfinance::interfaces::pair::{
        IStarkFinancePair, IStarkFinancePairCamelOnly, IFeesVaultDispatcherTrait, IFeesVaultDispatcher,
        IStarkFinanceCalleeDispatcherTrait, IStarkFinanceCalleeDispatcher, Snapshot, GlobalFeesAccum,
        RelativeFeesAccum,
    };
    use starkfinance::utils::pow::{pow};
    use starkfinance::token::erc20::{ERC20};
    ...
    fn constructor(
        ...
        if (stable) {
            ERC20::InternalImpl::initializer(ref erc20_state, 'sStarkFinance Pair', 'sSFN-P');
        } else {
            ERC20::InternalImpl::initializer(ref erc20_state, 'vStarkFinance Pair', 'vSFN-P');
        }
    ...
    #[external(v0)]
    impl StarkFinancePairImpl of IStarkFinancePair<ContractState> {
        ...
        fn get_amount_out(
            ...
            let pool_fee = IStarkFinanceFactoryABIDispatcher {
    ...
    #[external(v0)]
    impl StarkFinancePairCamelOnlyImpl of IStarkFinancePairCamelOnly<ContractState> {
        ...
        fn increaseAllowance(
            ...
            StarkFinancePairImpl::increase_allowance(ref self, spender, addedValue)
        }

        fn decreaseAllowance(
            ref self: ContractState, spender: ContractAddress, subtractedValue: u256
        ) -> bool {
            StarkFinancePairImpl::decrease_allowance(ref self, spender, subtractedValue)
        }
        ...
        fn transferFrom(
            ...
            StarkFinancePairImpl::transfer_from(ref self, sender, recipient, amount)
        }
    ...
    impl InternalFunctions of InternalFunctionsTrait {
        ...
        fn _update_global_fees(ref self: ContractState, amount0In: u256, amount1In: u256) {
            ...
            let factory = IStarkFinanceFactoryABIDispatcher { contract_address: self.factory() };
            ...
            if (amount0In > 0) {
                ...
                if (protocol_fee_on) {
                    let pfee0 = (fee0 * 5000) / FEE_DENOMINATOR; // 50% of fee0 to the protocol = 25% for burning + 25% tresury
                    IFeesVaultDispatcher {
                        contract_address: self.fee_vault()
                    }.update_protocol_fees(pfee0, 0); // update the protocol fees

                    let ufee0 = fee0 - pfee0; // 50% of fee0 to LP providers
    ...
    impl Modifiers of ModifiersTrait {
        ...
        fn _assert_not_paused(self: @ContractState) {
            let config = self.config.read();
            let factoryDipatcher = IStarkFinanceFactoryABIDispatcher { contract_address: config.factory };
        ...
        fn assert_only_handler(self: @ContractState) {
            ...
            let factory = IStarkFinanceFactoryABIDispatcher {
